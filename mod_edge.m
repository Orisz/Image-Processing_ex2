function [eout,thresh,gv_45,gh_135] = mod_edge(varargin)
...
if strcmp(method,'canny')
    ...
    %subimages & prep
    gcd_pic = gcd(size(a,1), size(a,2));
    m = size(a,1)/gcd_pic;
    n = size(a,2)/gcd_pic;
    block_mat = mat2cell(a,  repmat(gcd_pic, 1, m), repmat(gcd_pic, 1, n));
    dx = zeros(size(a,1), size(a,2));
    dy = dx;
    
    %operate smoothGradient on each block
    for i = 1:size(block_mat,1) 
        for j = 1:size(block_mat,2) 
            cur_block = cell2mat(block_mat(i, j));
            cur_sigma = mean(var(cur_block)) * 90;
            j_ind = gcd_pic * (j-1) + 1 : gcd_pic * j;
            i_ind = gcd_pic * (i-1) + 1 : gcd_pic * i;
            [dx(i_ind, j_ind), dy(i_ind, j_ind)] = smoothGradient(cur_block, cur_sigma);
        end
    end
